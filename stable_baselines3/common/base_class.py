...
class BaseHebbianAlgorithm(ABC):
    """
    The base of RL algorithms
    ...
    def __init__(
        ...
        # Create and wrap the env if needed
        if env is not None:
            post_env = maybe_make_env(env, self.verbose)
            ...
            post_env = self._wrap_env(post_env, self.verbose, monitor_wrapper)
    ...
    def _setup_learn(
        ...
        if self.ep_info_buffer is None or reset_num_timesteps:
            ...
            self.ep_info_buffer = deque(maxlen=100)
            self.ep_success_buffer = deque(maxlen=100)
            self.post_ep_info_buffer = deque(maxlen=100)
    ...
    def _update_info_post_buffer(self, infos: List[Dict[str, Any]], dones: Optional[np.ndarray] = None) -> None:
        """
        Retrieve reward, episode length, episode success and update the buffer
        if using Monitor wrapper or a GoalEnv.
        ...
        for idx, info in enumerate(infos):
            ...
            maybe_is_success = info.get("is_success")
            if maybe_ep_info is not None:
                self.post_ep_info_buffer.extend([maybe_ep_info])
            # if maybe_is_success is not None and dones[idx]:
            #     self.ep_success_buffer.append(maybe_is_success)
    ...
    def predict_conditioned(
        ...
        :return: the model's action and the next hidden state
            (used in recurrent policies)
        """
        return self.policy.predict_conditioned(observation, state, episode_start, deterministic,weights,biases)
