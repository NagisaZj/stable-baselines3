...
from stable_baselines3 import PPO,A2C,SAC,HebbianSAC
...
def main(argv):
    ...
    folder_path='./logs/'+'HebbianSAC/'+args.environment+'/'+time.asctime( time.localtime(time.time()) ).replace(' ','_')
    new_logger = configure(folder_path, ["stdout", "csv", "tensorboard",'log','json'])
    model = HebbianSAC("MlpPolicy", env, verbose=1,train_freq=(1,'episode'),gradient_steps=-1)
